...
def test_update_tag(user, tag):
    client.force_authenticate(user)
    response = client.put(reverse("tag_id", kwargs={"pk": 1}), data={"name": "2132"})
    assert response.status_code == 200
...
def test_post_tag(user):
    client.force_authenticate(user)
    response = client.post(reverse("tag_id", kwargs={"pk": 2}), data={"name": "tesf"})
...
def test_delete_tag(user, tag):
    client.force_authenticate(user)
    response = client.delete(reverse("tag_id", kwargs={"pk": 1}))
    assert response.status_code == 204


def test_retrieve_tag(user, tag):
    client.force_authenticate(user)
    response = client.get(reverse("tag_id", kwargs={"pk": 1}))
    assert response.status_code == 200
